...
1) Haz una función que añada un visitante nuevo:
	...
	El visitante irá a un área y una sección aleatoria,
	...
	for (var indAreas = 0 ; indAreas<zoo.areas.length; indAreas ++){
		...
		for(var indRecinto = 0 ; indRecinto<area.recintos.length; indRecinto++){
			...
			for(indAnimal = 0 ; indAnimal< recinto.animales.length; indAnimal++){
				...
				console.log(tab + tab + ">> " + indAnimal + ". Animal " + animal.nombre + " " + animal.emoji + " (Salud: " + animal.salud + ")");				
...
function ejecutarCicloAnimal(){
	...
	for(var indiceAnimales=0; indiceAnimales<listaAnimalesZoo.length; indiceAnimales++){
		...
		animal.actualizarEstadoAnimal(recinto, zoo);
	}
...
var zoo = {
	...
	tienda: [],
	ciclo:0,
	cerrarZoo: function (intervalID){
		window.clearInterval(intervalID);
		for(var indAreas = this.areas.length - 1 ; indAreas>=0; indAreas --){
			var area = this.areas[indAreas];
...
function crearAnimal(nombre, especie, salud, hambre, pais, emoji){
	return {
		...
		pais: pais,
		emoji:emoji,
		ganarPerderSaludAleatorio: function(){
			var factorDeSalud = 10;
			if(generarNumeroAleatorioEntre(0, 1)==0){
				factorDeSalud = -10
			}

			this.salud = this.salud + factorDeSalud;
		...
		aumentarHambre : function(){			
			...
			this.hambre = this.hambre + factorDeHambre;			
		},

		alimentarAnimal: function(zoo, recinto){
			var indiceAnimalAleatorio = 0;
			if((zoo.caja - 1000) > 0){
				zoo.caja = zoo.caja - 1000;
				this.hambre = 0;
				//console.log(this.nombre + " con hambre cero, alimentado por el Zoo");
			} else if(recinto.personas.length>0){
				//console.log("animal.hambre " + this.hambre);
				if(this.hambre>=150 && this.hambre<300 ){
					...
					//console.log(this.nombre + " se come a persona " + persona.nombre);
					recinto.personas.splice(indiceAleatorio,1);
					this.hambre = 0;
					//console.log(this.nombre + " con hambre cero, se comio una persona");
					...
					if(recinto.animales.length>1){//Solo si hay mas de 1 animal, para que no se coma asi mismo
						...
						do{
							...
							if(this != otroAnimal){//no se puede comer asi mismo
								recinto.animales.splice(indiceAnimalAleatorio,1);
							}
							
						}while (this == otroAnimal);	
					}
		...
		actualizarEstadoAnimal: function (recinto, zoo){
			//var animal = animalConRecinto.animal;
			//var recinto = animalConRecinto.recinto;
			...
			if (this.hambre>=100){
				//console.log(this.nombre + " debe ser alimentado, alimentando...");		

				this.alimentarAnimal(zoo, recinto);
			}

			this.ganarPerderSaludAleatorio();

			if (this.salud<50){
				//console.log(this.nombre + " debe ir a Enfermeria");
				...
				var indDeAnimalEnRecinto = animales.indexOf(this);
